...
export function useCars(filters: CarFilters = {}, page = 1) {
  ...
  const fetch = useCallback(async () => {
    ...
    if (error) {
      ...
      setError(error.message || 'Failed to load cars. Please check your Supabase connection.');
      setCars([]);
      setTotal(0);
    } else {
      setCars((data as Car[]) || []);
  ...
  return { cars, total, loading, error, pageSize: DEFAULT_PAGE_SIZE, refetch: fetch };
}
